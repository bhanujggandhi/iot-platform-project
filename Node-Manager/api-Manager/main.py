...
async def apps(app_name: str = Path(...)):
    ...
    if ret==None:
        return HTMLResponse(content='''<html><head><title>some title</title></head><body><h1>Invalid Application</h1></body></html>''', status_code=400)
    else:
        if ret['active'] == False:
            return HTMLResponse(content='''<html><head><title>some title</title></head><body><h1>Application is not Running</h1></body></html>''', status_code=400)
        else:
